...
class When
{
	 ...
	 * @param string $frequency of the recursion, valid frequencies: secondly, minutely, hourly, daily, weekly, monthly, yearly
	 */
	public function recur($start_date, $frequency = "daily")
	{
		try
		{
			...
			else
			{
				...
				if ('Z' == substr($start_date,-1)) {
					$timezone = new DateTimeZone('UTC');
					$start_date = trim($start_date, 'Z');
				}
				$this->start_date = new DateTime($start_date, @$timezone);
			}
	...
	public function rrule($rrule)
	{
		...
		{
			list($rule, $param) = array_map('strtoupper', explode("=", $part));
	...
	protected function create_suggestions()
	{
		...
		if (!is_object($this->try_date)) throw new WhenException('No try date established - has start time/date been set by calling recur()?');
